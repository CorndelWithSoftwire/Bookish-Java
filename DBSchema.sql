...
CREATE TABLE `Authors` (`AuthorId` int  NOT NULL, `FirstName` varchar(32)  NOT NULL, `LastName` varchar(32)  NOT NULL, PRIMARY KEY (`AuthorId`));

CREATE TABLE `Categories` (`CategoryId` int  NOT NULL , `Category` varchar(32)  NULL, PRIMARY KEY (`CategoryId`));
...
ALTER TABLE `Book` ADD CONSTRAINT `fk_Book_BookId` FOREIGN KEY(`BookId`) REFERENCES `BookAuthor` (`Book`);
...
CREATE INDEX `idx_Authors_FirstName` ON `Authors` (`FirstName`);
